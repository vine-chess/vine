...
f32 PolicyContext::logit(Move move, PieceType moving_piece) const {
    ...
    for (usize i = 0; i < L1_SIZE / 2 / VECTOR_SIZE; ++i) {
        const auto first_clamped =
            util::min<i16, VECTOR_SIZE>(util::max<i16, VECTOR_SIZE>(feature_accumulator_[i], zero), one);
        const auto second_clamped = util::min<i16, VECTOR_SIZE>(
            util::max<i16, VECTOR_SIZE>(feature_accumulator_[i + L1_SIZE / 2 / VECTOR_SIZE], zero), one);
